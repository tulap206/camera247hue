...
export async function POST(request: Request) {
  ...
  try {
    ...
    for (let i = 0; i < posts.length; i++) {
      ...
      // Location & Ward (40 Wards & Communes in Hue)
      const loc = knownInfo.addr || p.location || 'TP. Huế'
      let district = 'Phường Phú Hội'
      const combinedLoc = `${loc} ${p.title} ${clientName}`.toLowerCase()

      if (/an cựu|an cuu/i.test(combinedLoc)) district = 'Phường An Cựu'
      else if (/an đông|an dong/i.test(combinedLoc)) district = 'Phường An Đông'
      else if (/an hòa|an hoa/i.test(combinedLoc)) district = 'Phường An Hòa'
      else if (/an tây|an tay/i.test(combinedLoc)) district = 'Phường An Tây'
      else if (/đông ba|dong ba|trần hưng đạo/i.test(combinedLoc)) district = 'Phường Đông Ba'
      else if (/gia hội|gia hoi|chi lăng/i.test(combinedLoc)) district = 'Phường Gia Hội'
      else if (/hương an|huong an/i.test(combinedLoc)) district = 'Phường Hương An'
      else if (/hương chữ|huong chu/i.test(combinedLoc)) district = 'Phường Hương Chữ'
      else if (/hương hồ|huong ho/i.test(combinedLoc)) district = 'Phường Hương Hồ'
      else if (/hương long|huong long|thiên mụ|văn thánh/i.test(combinedLoc)) district = 'Phường Hương Long'
      else if (/hương sơ|huong so/i.test(combinedLoc)) district = 'Phường Hương Sơ'
      else if (/hương văn|huong van/i.test(combinedLoc)) district = 'Phường Hương Văn'
      else if (/hương vinh|huong vinh/i.test(combinedLoc)) district = 'Phường Hương Vinh'
      else if (/hương xuân|huong xuan/i.test(combinedLoc)) district = 'Phường Hương Xuân'
      else if (/kim long/i.test(combinedLoc)) district = 'Phường Kim Long'
      else if (/phú hậu|phu hau/i.test(combinedLoc)) district = 'Phường Phú Hậu'
      else if (/phú hội|phu hoi|hương giang|chu văn an|nguyễn thái học|võ thị sáu|phạm ngũ lão|nguyễn huệ/i.test(combinedLoc)) district = 'Phường Phú Hội'
      else if (/phú nhuận|phu nhuan|lý thường kiệt|đống đa|hai bà trưng/i.test(combinedLoc)) district = 'Phường Phú Nhuận'
      else if (/phú thượng|phu thuong|royal park/i.test(combinedLoc)) district = 'Phường Phú Thượng'
      else if (/phước vĩnh|phuoc vinh|trần phú|đặng huy trứ/i.test(combinedLoc)) district = 'Phường Phước Vĩnh'
      else if (/phường đúc|phuong duc|bùi thị xuân/i.test(combinedLoc)) district = 'Phường Phường Đúc'
      else if (/tây lộc|tay loc|lê đại hành/i.test(combinedLoc)) district = 'Phường Tây Lộc'
      else if (/thuận an|thuan an/i.test(combinedLoc)) district = 'Phường Thuận An'
      else if (/thuận hòa|thuan hoa|nguyễn trãi/i.test(combinedLoc)) district = 'Phường Thuận Hòa'
      else if (/thuận lộc|thuan loc|đinh tiên hoàng/i.test(combinedLoc)) district = 'Phường Thuận Lộc'
      else if (/thủy biều|thuy bieu/i.test(combinedLoc)) district = 'Phường Thủy Biều'
      else if (/thủy vân|thuy van|ecogarden|eco garden/i.test(combinedLoc)) district = 'Phường Thủy Vân'
      else if (/thủy xuân|thuy xuan|vũ ngọc phan|lê ngô cát/i.test(combinedLoc)) district = 'Phường Thủy Xuân'
      else if (/trường an|truong an|điện biên phủ/i.test(combinedLoc)) district = 'Phường Trường An'
      else if (/vĩnh ninh|vinh ninh|lê lợi|hà nội|hoàng hoa thám/i.test(combinedLoc)) district = 'Phường Vĩnh Ninh'
      else if (/vỹ dạ|vy da|nguyễn sinh cung|cồn hến|phạm văn đồng/i.test(combinedLoc)) district = 'Phường Vỹ Dạ'
      else if (/xuân phú|xuan phu|tố hữu|bà triệu|hà huy tập|hoàng quốc việt/i.test(combinedLoc)) district = 'Phường Xuân Phú'
      else if (/hải dương|hai duong/i.test(combinedLoc)) district = 'Xã Hải Dương'
      else if (/hương phong|huong phong/i.test(combinedLoc)) district = 'Xã Hương Phong'
      else if (/hương thọ|huong tho/i.test(combinedLoc)) district = 'Xã Hương Thọ'
      else if (/phú dương|phu duong/i.test(combinedLoc)) district = 'Xã Phú Dương'
      else if (/phú mậu|phu mau/i.test(combinedLoc)) district = 'Xã Phú Mậu'
      else if (/phú mỹ|phu my/i.test(combinedLoc)) district = 'Xã Phú Mỹ'
      else if (/phú thanh|phu thanh/i.test(combinedLoc)) district = 'Xã Phú Thanh'
      else if (/thủy bằng|thuy bang/i.test(combinedLoc)) district = 'Xã Thủy Bằng'
